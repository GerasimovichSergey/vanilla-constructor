...
const createHeader = ({title, header: {logo, social, menu}}) => {
    // const {title, header} = param;
    // const {logo, social, menu} = header;
...
const createFooter = ({footer: {copyright, menu}}) => {
    ...
    if (copyright) {
        const copyElem = getElement('span', ['copyright']);
        copyElem.textContent = copyright;
        footerLeft.append(copyElem);
    }

    if (menu) {
        const menuElem = getElement('nav', ['footer-menu']);
        ...
        footerRight.append(menuElem);
        menuElem.append(...allMenuLink);
    }
